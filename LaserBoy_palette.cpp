...
//############################################################################
int LaserBoy_palette::from_ifstream_ild(ifstream& in, const LaserBoy_ild_header& header)
{
    int             i;
                                        ...
                                        )
{
    ...
    LaserBoy_color  color;
    clear();
    while(    color.from_ifstream_txt(in, format_1, line_number)
           && i--
         )
        push_back(color);
    if(size())
...
//############################################################################
bool LaserBoy_palette::save_as_txt(string file) const
{
...
//############################################################################
int LaserBoy_palette::to_bmp_palette(struct LaserBoy_bmp* bmp) const
{
    int i;
    if(bmp->bpp == 8)
    {
    ...
    return LASERBOY_INCOMPATIBLE_FORMATS;
}
...
void LaserBoy_palette::find_factors()
{
    ...
    //------------------------------------------------------------------------
    for(i = 0; i < number_of_colors(); i++)
        if(p_space->bmp.make_rgb(at(i).r, at(i).g, at(i).b) > 0)
        {
            first = i;
            break;
        }
...
void LaserBoy_palette::best_reduction()
{
    if(size())
    {
        ...
        for(k = 1; k < 765; k++) // minimum difference
        {
            ...
            if((int)size() <= LASERBOY_PALETTE_MAX)
                break;
        }
        //--------------------------------------------------------------------
        find_factors();
