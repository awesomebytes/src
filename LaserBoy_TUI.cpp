...
void LaserBoy_TUI::overlay_palettes()
{
    ...
    {
        for(i = 0; i < const_palette_picker(palette_index).number_of_colors(); i++)
            for(j = 0; j < font_size_factor; j++)
                bmp_put_line_fast(&bmp,
                                  ...
                                  x_off + 50,
                                  i * font_size_factor + j,
                                  bmp.make_rgb(const_palette_picker(palette_index)[i].r,
                                               const_palette_picker(palette_index)[i].g,
                                               const_palette_picker(palette_index)[i].b
                                              )
                                 );
    }
    //------------------------------------------------------------------------
    if(palette_index == LASERBOY_TRUE_COLOR || show_target_palette)
        for(i = 0; i < const_palette_picker(target_palette_index).number_of_colors(); i++) // target palette
            for(j = 0; j < font_size_factor; j++)
                bmp_put_line_fast(&bmp,
                                  x_off + 70,
                                  i * font_size_factor + j,
                                  x_off + 120,
                                  i * font_size_factor + j,
                                  bmp.make_rgb(const_palette_picker(target_palette_index)[i].r,
                                               const_palette_picker(target_palette_index)[i].g,
                                               const_palette_picker(target_palette_index)[i].b
                                              )
                                 );
    //------------------------------------------------------------------------
    if(show_color_cursor) // selected_color_index example line
    {
        if(palette_index != LASERBOY_TRUE_COLOR)
            bmp_put_line_fast(&bmp,
                              ...
                              bmp.make_rgb(const_palette_picker(palette_index)[selected_color_index].r,
                                           const_palette_picker(palette_index)[selected_color_index].g,
                                           const_palette_picker(palette_index)[selected_color_index].b
                                          )
                             );
        //--------------------------------------------------------------------
        if(palette_index == LASERBOY_TRUE_COLOR || show_target_palette)
            bmp_put_line_fast(&bmp,
                              ...
                              x_off + 63,
                              LASERBOY_PALETTE_MAX * font_size_factor,
                              bmp.make_rgb(const_palette_picker(target_palette_index)[selected_color_index].r,
                                           const_palette_picker(target_palette_index)[selected_color_index].g,
                                           const_palette_picker(target_palette_index)[selected_color_index].b
                                          )
                             );
        //--------------------------------------------------------------------
    ...
    if(palette_index == LASERBOY_TRUE_COLOR)
        ...
        bmp_printf(   &bmp
                    ...
                    , 0
                    , font_size_factor
                    , const_palette_picker(palette_index).name.c_str()
                  );
    //------------------------------------------------------------------------
    if(palette_index == LASERBOY_TRUE_COLOR || show_target_palette)
        bmp_printf(   &bmp // target palette name
                    ...
                    , 1
                    , font_size_factor
                    , const_palette_picker(target_palette_index).name.c_str()
                  );
    //------------------------------------------------------------------------
    if(show_color_cursor)
    {
        if(palette_index == LASERBOY_TRUE_COLOR || show_target_palette)
        {
            ...
            bmp_printf(   &bmp // target palette name
                        ...
                        , font_size_factor
                        , "    %3d"
                        , const_palette_picker(target_palette_index)[selected_color_index].r
                      );
            bmp_printf(   &bmp // target palette name
                        , x_off + 130
                        , y_line * 10
        ...
        bmp_printf(   &bmp // selected color red value
                    ...
                    , font_size_factor
                    , "    %3d"
                    , const_palette_picker(palette_index)[selected_color_index].r
                  );
...
{
    int   i,
          vectors      = (int)current_frame().size() - 1,
          from_vertex  = 0,
    ...
    if(current_frame().number_of_vertices())
    {
        ...
        bmp_printf(&bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor,
        ...
        y_off += y_line;
        bmp_printf(&bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor,
                   "lit %d : %0.2f%% dark %d : %0.2f%%",
                   lit,
                   (lit/(double)vectors) * 100.0,
                   blank,
                   (blank/(double)vectors) * 100.0
                  );
        y_off += y_line;
        bmp_printf(&bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor,
        ...
        y_off += y_line;
        bmp_printf(&bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor,
                   "avg %0.2f : total %0.2f : farthest %d %d %d",
                   current_frame().total_distance() / vectors,
        ...
        y_off += y_line;
        bmp_printf(&bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor,
                   "max angle %0.2f%c : total angle %0.2f%c",
        ...
        if(show_cursors)
        {
            ...
            bmp_printf(&bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor,
                       ...
                       current_frame().selected_segment().find_center().x,
                       current_frame().selected_segment().find_center().y,
                       current_frame().selected_segment().find_center().z,
...
void LaserBoy_TUI::overlay_frame_set_stats()
{
    ...
    bmp_printf(&bmp, x_off, y_off, bmp.white, bmp_bg_color, 4, font_size_factor,
               "frame set stats"
              );
    y_off += y_line;
    bmp_printf(&bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor,
...
void LaserBoy_TUI::overlay_out_txt_menu()
{
    ...
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "    1 txt save current frame"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "    2 txt save selected frames"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "    3 txt save all frames"
              );
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "    4 txt save current frame palette"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "    5 txt save target palette"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "    6 txt save LaserBoy wave color rescales"
              );
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
...
void LaserBoy_TUI::overlay_value_settings_menu()
{
    ...
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, bmp.make_rgb(0,0,255), bmp_bg_color, 4, font_size_factor
                , "                                                       %3d"
                , bg_color.b
              );
    bmp_printf(   &bmp, x_off, y_off, bmp.make_rgb(0,255,0), bmp_bg_color, 4, font_size_factor
                , "                                                  %3d"
                , bg_color.g
              );
    bmp_printf(   &bmp, x_off, y_off, bmp.make_rgb(255,0,0), bmp_bg_color, 4, font_size_factor
                , "                                             %3d"
                , bg_color.r
              );
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
...
void LaserBoy_TUI::overlay_visuals_menu()
{
    ...
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_vertices)?(" --> 1 vertices"):("     1 vertices"))
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_blanking)?(" --> 2 blanking"):("     2 blanking"))
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_intro)?(" --> 3 intro"):("     3 intro"))
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_bridge)?(" --> 4 bridge"):("     4 bridge"))
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_coda)?(" --> 5 coda"):("     5 coda"))
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_indices)?(" --> 6 indices"):("     6 indices"))
              );
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_palette)?(" --> 7 palette"):("     7 palette"))
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_target_palette)?(" --> 8 target palette"):("     8 target palette"))
              );
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_stats)?(" --> 9 stats"):("     9 stats"))
              );
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_fixed_bounds)?(" --> 0 fixed bounds"):("     0 fixed bounds"))
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_fixed_origin)?(" --> a fixed origin"):("     a fixed origin"))
              );
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_floating_axis)?(" --> b floating axis"):("     b floating axis"))
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_floating_bounds)?(" --> c floating bounds"):("     c floating bounds"))
              );
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_onion_skin)?(" --> d onion skin"):("     d onion skin"))
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_onion_visuals)?(" --> e onion visuals"):("     e onion visuals"))
              );
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_Z_as_order)?(" --> f display vector order in Z"):("     f display vector order in Z"))
              );
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_quad_view)?(" --> g quad orthographic view"):("     g quad orthographic view"))
              );
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , ((show_TUI_clue)?(" --> h TUI clue"):("     h TUI clue"))
              );
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "     i show | hide all visuals"
              );
    //------------------------------------------------------------------------
    y_off += y_line;
    y_off += y_line;
...
void LaserBoy_TUI::overlay_switch_settings_menu()
{
    ...
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , (   (auto_apply_offsets)
                    ? (" --> f apply system offsets to wave")
                    : ("     f apply system offsets to wave")
                  )
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , (   (auto_apply_rescales)
                    ? (" --> g apply system color rescales to wave")
                    : ("     g apply system color rescales to wave")
                  )
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , (   (channels_of_wav_out == 8)
                    ? (" --> h save waves with stereo audio silence (8ch)")
                    : ("     h save waves with stereo audio silence (8ch)")
                  )
              );
    y_off += y_line;
    ...
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , (   (save_txt_with_color)
                    ? (" --> i save txt frames with integrated color")
                    : ("     i save txt frames with integrated color")
                  )
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , (   (save_txt_unit)
                    ? (" --> j save txt in unit coordinates (-1.0 to +1.0)")
                    : ("     j save txt in unit coordinates (-1.0 to +1.0)")
                  )
              );
    y_off += y_line;
    ...
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , (   (save_txt_named_palettes)
                    ? (" --> l save txt named LaserBoy palette values")
                    : ("     l save txt named LaserBoy palette values")
                  )
              );
    //------------------------------------------------------------------------
...
void LaserBoy_TUI::overlay_palette_tranz_menu()
{
    ...
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "    b c set to target by best match | index"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
    ...
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "    o O copy                                current | target"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "    q Q add new color to                    current | target"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "    r R set selected color RGB in           current | target"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "    s S insert color RGB before selected in current | target"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "    t T omit color selected in              current | target"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "    u U omit selected color to end of       current | target"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
...
void LaserBoy_TUI::overlay_frame_set_tranz_menu()
{
    ...
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      1 omit selected frame(s)"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      2 trim to selected frame(s)"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      3 omit repeats from formatted wave import"
              );
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      4 copy selected frame(s) to begining"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      5 copy selected frame(s) to end"
              );
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      6 move selected frame(s) to begining"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      7 move selected frame(s) to end"
              );
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      8 add empty frame to begining"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      9 add empty frame after current"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      0 add empty frame to end"
              );
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      a reverse frame set order"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      b reverse order of selected frames"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      c make current frame first"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      d randomize frame set order"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      e omit every n-th frame"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      f omit all but every n-th frame"
              );
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
...
void LaserBoy_TUI::overlay_frame_tranz_menu()
{
    ...
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      f reduce lit vectors"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      g blank equivalent vectors"
              );
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      h bond segments"
    ...
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, next_hue(), bmp_bg_color, 4, font_size_factor
                , "      r rename this frame | all frames"
              );
    y_off += y_line;
    y_off += y_line;
    bmp_printf(   &bmp, x_off, y_off, bmp.white, bmp_bg_color, 4, font_size_factor
...
//            red_map[i] = green_map[i] = blue_map[i] = 0;
        ...
        {
            if(abs(sample_1.channel[0] - sample_0.channel[0]) > max_diff_x)
            {
                max_diff_x = abs(sample_1.channel[0] - sample_0.channel[0]);
                max_x_index = sample_count;
            }
            if(abs(sample_1.channel[1] - sample_0.channel[1]) > max_diff_y)
            {
...
int LaserBoy_TUI::overlay_file_list(string start)
{
    ...
    bmp_printf(&bmp, x_off, 0, bmp.white, bmp_bg_color, 4, font_size_factor,
               "%s                  ",
    ...
    if(files_shown == 0)
    {
        bmp_printf(&bmp, x_off, y_off, bmp.white, bmp_bg_color, 4, font_size_factor, "none found        ");
