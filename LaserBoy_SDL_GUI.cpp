...
//############################################################################
string LaserBoy_SDL_GUI::display_prompt_file_with_auto_complete(string prompt, string part)
{
    bool   done         = false;
...
//############################################################################
string LaserBoy_SDL_GUI::display_prompt_dir_with_auto_complete(string prompt)
{
    bool   done         = false;
    int    x_off        = LASERBOY_STATS_DISPLAY_OFFSET * space.font_size_factor,
           y_off        = space.bmp.yres - 24 * space.font_size_factor,
...
//############################################################################
string LaserBoy_SDL_GUI::display_prompt_f_effect_with_auto_complete(string prompt)
{
    bool   done         = false;
    int    x_off        = LASERBOY_STATS_DISPLAY_OFFSET * space.font_size_factor,
           y_off        = space.bmp.yres - 24 * space.font_size_factor,
...
//############################################################################
string LaserBoy_SDL_GUI::display_prompt_fs_effect_with_auto_complete(string prompt)
{
    bool   done         = false;
    int    x_off        = LASERBOY_STATS_DISPLAY_OFFSET * space.font_size_factor,
           y_off        = space.bmp.yres - 24 * space.font_size_factor,
...
//############################################################################
string LaserBoy_SDL_GUI::display_prompt_and_echo_name(string prompt)
{
    bool   done         = false;
    int    x_off        = LASERBOY_STATS_DISPLAY_OFFSET * space.font_size_factor,
           y_off        = space.bmp.yres - 24 * space.font_size_factor,
...
//############################################################################
string LaserBoy_SDL_GUI::display_prompt_and_echo_string(string prompt)
{
    bool   done         = false;
    int    x_off        = LASERBOY_STATS_DISPLAY_OFFSET * space.font_size_factor,
           y_off        = space.bmp.yres - 24 * space.font_size_factor,
    ...
    bmp_printf(&(space.bmp),
               x_off,
               y_off,
               space.bmp.make_rgb(0,255,255),bmp_bg_color,
               0,
               space.font_size_factor,
               "_"
              );
...
//############################################################################
double LaserBoy_SDL_GUI::display_prompt_and_echo_double(string prompt, double value)
{
    bool   done         = false;
    int    x_off        = LASERBOY_STATS_DISPLAY_OFFSET * space.font_size_factor,
           y_off        = space.bmp.yres - 24 * space.font_size_factor,
...
//############################################################################
int LaserBoy_SDL_GUI::display_prompt_and_echo_int(string prompt, int default_value)
{
    bool   done         = false;
    int    x_off        = LASERBOY_STATS_DISPLAY_OFFSET * space.font_size_factor,
           y_off        = space.bmp.yres - 24 * space.font_size_factor,
...
//############################################################################
int LaserBoy_SDL_GUI::display_prompt_and_echo_nibble(string prompt)
{
    bool   done         = false;
    int    x_off        = LASERBOY_STATS_DISPLAY_OFFSET * space.font_size_factor,
           y_off        = space.bmp.yres - 24 * space.font_size_factor,
...
//############################################################################
bool LaserBoy_SDL_GUI::display_prompt_and_echo_bool(string prompt)
{
    int    x_off  = LASERBOY_STATS_DISPLAY_OFFSET * space.font_size_factor,
           y_off  = space.bmp.yres - 24 * space.font_size_factor;
...
//############################################################################
void LaserBoy_SDL_GUI::display_error(string err)
{
    space.TUI_clue = "oops!";
    //------------------------------------------------------------------------
    bmp_printf(   &(space.bmp)
...
//############################################################################
void LaserBoy_SDL_GUI::wait_4_key()
{
    while(true)
    {
        SDL_WaitEvent(&event);
...
//############################################################################
void LaserBoy_SDL_GUI::check_for_SDL_arrow_keys(SDLKey key)
{
    switch(key)
    {
        default:
...
void LaserBoy_SDL_GUI::system_settings_menu()
{
    ...
    while(in_menu)
    {
        ...
        if(event.type == SDL_KEYDOWN)
        {
            ...
            {
                default:
                    check_for_SDL_arrow_keys(event.key.keysym.sym);
                    break;
                //------------------------------------------------------------
                case SDLK_ESCAPE:
                    in_menu = false;
                ...
                //------------------------------------------------------------
                case 'p':
                    temp_double = display_prompt_and_echo_int("onion skin fade %");
                    if(prompt_escaped)
                        break;
...
void LaserBoy_SDL_GUI::bit_resolution_menu()
{
    ...
    do
    {
        ...
        if(prompt_escaped)
            break;
        space.signal_bit_mask[0] = temp_int;
        //--------------------------------------------------------------------
        space.clean_screen();
        space.render_space();
        space.overlay_bit_resolution_settings();
        ...
        if(prompt_escaped)
            break;
        space.signal_bit_mask[1] = temp_int;
        //--------------------------------------------------------------------
        space.clean_screen();
        space.render_space();
        space.overlay_bit_resolution_settings();
        ...
        if(prompt_escaped)
            break;
        space.signal_bit_mask[5] = temp_int;
        //--------------------------------------------------------------------
        space.clean_screen();
        space.render_space();
        space.overlay_bit_resolution_settings();
        ...
        if(prompt_escaped)
            break;
        space.signal_bit_mask[2] = temp_int;
        //--------------------------------------------------------------------
        space.clean_screen();
        space.render_space();
        space.overlay_bit_resolution_settings();
        ...
        if(prompt_escaped)
            break;
        space.signal_bit_mask[3] = temp_int;
        //--------------------------------------------------------------------
        space.clean_screen();
        space.render_space();
        space.overlay_bit_resolution_settings();
        ...
        if(prompt_escaped)
            break;
        space.signal_bit_mask[4] = temp_int;
        //--------------------------------------------------------------------
        space.clean_screen();
        space.render_space();
        space.overlay_bit_resolution_settings();
        ...
        if(prompt_escaped)
            break;
        space.signal_bit_mask[6] = temp_int;
        //--------------------------------------------------------------------
        space.clean_screen();
        space.render_space();
        space.overlay_bit_resolution_settings();
        ...
        if(prompt_escaped)
            break;
        space.signal_bit_mask[7] = temp_int;
    } while(0);
    return;
}
    ...
    while(in_menu)
    {
        ...
        if(event.type == SDL_KEYDOWN)
        {
            switch(event.key.keysym.unicode)
            {
                ...
                //------------------------------------------------------------
                case 'o':
                    space.current_frame().rotate_around_origin();
                ...
                case 'O':
                    ...
                    space.render_space();
                    display_space();
                    space.rotate_around_origin();
                    space.view_angle = 0.0;
                    space.TUI_clue = "set rotate all";
                    break;
                //------------------------------------------------------------
                case 't':
                    space.current_frame().move();
                    space.view_offset = 0;
                ...
                //------------------------------------------------------------
                case 'g':
                    space.current_frame().scale_around_origin();
                    space.view_scale = 1.0;
                ...
                //------------------------------------------------------------
                case 'H':
                    space.TUI_clue = "setting view all";
                    space.render_space();
                    display_space();
                    space.scale_around_origin();
                    space.rotate_around_origin();
    ...
    while(in_menu)
    {
        ...
        if(event.type == SDL_KEYDOWN)
        {
            switch(event.key.keysym.unicode)
            {
                ...
                //------------------------------------------------------------
                case 'G':
                    space.TUI_clue = "24b-it to palette all";
                    space.render_space();
                    display_space();
                ...
                //------------------------------------------------------------
                case 'o':
                    space.copy_palette(space.palette_index);
                    break;
                //------------------------------------------------------------
                case 'O':
                ...
                //------------------------------------------------------------
                case 'q':
                    add_color_to_current_palette_menu();
                    break;
                //------------------------------------------------------------
                case 'Q':
                ...
                case 'p':
                    ...
                    if(space.selected_color_index >= 255)
                ...
                case 'P':
                    ...
                    if(space.selected_color_index < 0)
                        space.selected_color_index = 255;
                    space.TUI_clue = "- color index";
                    break;
                //------------------------------------------------------------
                case 'r':
                    edit_color_in_current_palette_menu();
                    break;
                //------------------------------------------------------------
                case 'R':
                ...
                //------------------------------------------------------------
                case 's':
                    insert_color_in_current_palette_menu();
                    break;
                //------------------------------------------------------------
                case 'S':
                ...
                //------------------------------------------------------------
                case 't':
                    if(space.palette_index < LASERBOY_BUILT_IN_PALETTES)
                    {
                        space.add_palette();
                        string name = space.target_palette().name;
                        space.target_palette() = space.current_palette();
                        space.target_palette().name = name;
                        space.current_frame().best_match_palette(space.number_of_palettes() - 1);
                        space.target_palette().erase_index(space.selected_color_index);
                    }
                    else
                        space.current_palette().erase_index(space.selected_color_index);
                    space.current_frame().sync_rgb_and_palette();
                    break;
                //------------------------------------------------------------
                case 'T':
                ...
                case 'u':
                    if(space.palette_index < LASERBOY_BUILT_IN_PALETTES)
                    {
                        ...
                        space.current_palette().clear_from_index(space.selected_color_index);
                    space.current_frame().sync_rgb_and_palette();
                    break;
                //------------------------------------------------------------
                case 'U':
...
//############################################################################
void LaserBoy_SDL_GUI::add_color_to_current_palette_menu()
{
    if(space.palette_index == LASERBOY_TRUE_COLOR)
    {
        display_error("current frame is 24-bit");
        wait_4_key();
        return;
    }
    ...
    if(space.palette_index < LASERBOY_BUILT_IN_PALETTES)
    {
        ...
        space.current_palette().add_color(LaserBoy_color((u_char)r, (u_char)g, (u_char)b));
...
//############################################################################
void LaserBoy_SDL_GUI::insert_color_in_current_palette_menu()
{
    if(space.palette_index == LASERBOY_TRUE_COLOR)
    {
        display_error("current frame is 24-bit");
    ...
    if(space.palette_index < LASERBOY_BUILT_IN_PALETTES)
    {
        ...
        space.current_palette().insert_color(LaserBoy_color((u_char)r, (u_char)g, (u_char)b), space.selected_color_index);
    space.current_frame().sync_rgb_and_palette();
    space.TUI_clue = "new color";
    return;
}
...
//############################################################################
void LaserBoy_SDL_GUI::edit_color_in_current_palette_menu()
{
    if(space.palette_index == LASERBOY_TRUE_COLOR)
    {
        display_error("current frame is 24-bit");
    ...
    if(space.palette_index < LASERBOY_BUILT_IN_PALETTES)
    {
        ...
        space.current_palette().set_index_color(space.selected_color_index, r, g, b);
    space.current_frame().sync_rgb_and_palette();
    ...
    while(in_menu)
    {
        ...
        if(event.type == SDL_KEYDOWN)
        {
            switch(event.key.keysym.unicode)
            {
                ...
                //------------------------------------------------------------
                case 'g':
                    space.current_frame().blank_equivalent_vectors();
                ...
                case 'G':
                    ...
                    space.render_space();
                    display_space();
                    space.blank_equivalent_vectors();
                    space.TUI_clue = "removed equal vectors";
                    break;
                //------------------------------------------------------------
                case 'h':
                ...
                //------------------------------------------------------------
                case 'j':
                    space.current_frame().fracture_segments();
                ...
                case 'l':
                {
                    ...
                    origin.push_back(LaserBoy_vertex());
                    space.current_frame().reorder_segments(origin);
                ...
                case 'L':
                    ...
                    space.render_space();
                    display_space();
                    space.reorder_segments();
                    space.TUI_clue = "reordered segments all";
                    break;
                //------------------------------------------------------------
                case 'm':
                    space.current_frame().add_dwell();
                ...
                //------------------------------------------------------------
                case 'n':
                    space.current_frame().add_lit_span_vertices();
                ...
                case 'N':
                    ...
                    space.render_space();
                    display_space();
                    space.add_lit_span_vertices();
                    space.TUI_clue = "added lit distance span all";
                    break;
                //------------------------------------------------------------
                case 'o':
                    space.current_frame().add_blank_span_vertices();
                ...
                case 'O':
                    ...
                    space.render_space();
                    display_space();
                    space.add_blank_span_vertices();
                    space.TUI_clue = "added blank distance span all";
                    break;
                //------------------------------------------------------------
                case 'p':
                    space.current_frame().enhance_dots();
                ...
                case 'q':
                    space.current_frame().minimize(0);
                    space.current_frame().optimize(0);
        ...
        if(event.type == SDL_KEYDOWN)
        {
            switch(event.key.keysym.unicode)
            {
                ...
                case '6':
                    ...
                    display_space();
                    {
                        LaserBoy_frame_set temp(space.selected_frames());
                        space.delete_selected_frames();
                        temp += space;
                        space = temp;
                    }
                ...
                case '8':
                    {
                        LaserBoy_frame_set temp(&space);
                        temp += LaserBoy_frame(&space, space.target_palette_index, true); // not quite an empty frame!
                        space = temp + space;
                    }
                ...
                case '9':
                    {
                        ...
                        for(i = 0; i <= space.frame_index; i++)
                            temp += space[i];
                        temp += LaserBoy_frame(&space, space.target_palette_index, true);
                ...
                //------------------------------------------------------------
                case '0':
                    space += LaserBoy_frame(&space, space.target_palette_index, true);
                ...
                case '<': // collapse selected frames
                    space.collapse_selected_frames();
    ...
    while(in_menu)
    {
        ...
        if(event.type == SDL_KEYDOWN)
        {
            switch(event.key.keysym.unicode)
            {
                ...
                //------------------------------------------------------------
                case'\\':
                    space.current_frame().spider = (space.current_frame().number_of_vertices() - 1);
                ...
                case 'P':
                    space.selected_color_index--;
                    if(space.selected_color_index < 0)
                        space.selected_color_index = space.current_palette().number_of_colors() - 1;
                    space.TUI_clue = "- color index";
                    break;
                //------------------------------------------------------------
                case 'a':
                ...
                //------------------------------------------------------------
                case '.':
                    space.current_frame().add_new_vertex(space.current_frame().spider);
                    if(space.current_frame().spider < (space.current_frame().number_of_vertices() - 1))
                ...
                //------------------------------------------------------------
                case '>':
                    space.current_frame().break_segment();
                ...
                //------------------------------------------------------------
                case 'l':
                    space.current_frame().connect_the_dots();
    ...
    while(in_menu)
    {
        ...
        if(event.type == SDL_KEYDOWN)
        {
            switch(event.key.keysym.unicode)
            {
                ...
                //------------------------------------------------------------
                case 'c':
                    space.current_frame() += space.current_frame().selected_segment();
                ...
                case 'q':
                    ...
                    if(prompt_escaped)
                        break;
                    space.current_frame().move_selection(LaserBoy_3D_double(offset - space.current_frame().selected_segment().find_center().x,
                                                                            0,
                                                                            0
                                                                           )
                                                        );
                ...
                case 'w':
                    ...
                    if(prompt_escaped)
                        break;
                    space.current_frame().move_selection(LaserBoy_3D_double(0,
                                                                            offset - space.current_frame().selected_segment().find_center().y,
                                                                            0
                                                                           )
                                                        );
                    break;
                ...
                case 'e':
                    ...
                    space.current_frame().move_selection(LaserBoy_3D_double(0,
                                                                            0,
                                                                            offset - space.current_frame().selected_segment().find_center().z
                                                                           )
                                                        );
                    break;
                //------------------------------------------------------------
                case 'x':
                    space.current_frame().move_selection(LaserBoy_3D_double(space.displacement_step, 0, 0));
                ...
                //------------------------------------------------------------
                case 'y':
                    space.current_frame().move_selection(LaserBoy_3D_double(0, space.displacement_step, 0));
                ...
                //------------------------------------------------------------
                case 'z':
                    space.current_frame().move_selection(LaserBoy_3D_double(0, 0, space.displacement_step));
                ...
                //------------------------------------------------------------
                case 'f':
                    space.current_frame().rotate_selection(LaserBoy_3D_double(0, 0, space.rotation_step));
                ...
                //------------------------------------------------------------
                case 'r':
                    space.current_frame().rotate_selection_on_fulcrum(LaserBoy_3D_double(0, 0, space.rotation_step));
                ...
                //------------------------------------------------------------
                case '9':
                    space.current_frame().scale_selection(LaserBoy_3D_double(space.magnitude_step_up, 1, 1));
                    space.TUI_clue = "+ scale X";
                    break;
                //------------------------------------------------------------
                case '(':
                    space.current_frame().scale_selection(LaserBoy_3D_double(space.magnitude_step_dn, 1, 1));
                    space.TUI_clue = "- scale X";
                    break;
                //------------------------------------------------------------
                case '0':
                    space.current_frame().scale_selection(LaserBoy_3D_double(1, space.magnitude_step_up, 1));
                    space.TUI_clue = "+ scale Y";
                    break;
                //------------------------------------------------------------
                case ')':
                    space.current_frame().scale_selection(LaserBoy_3D_double(1, space.magnitude_step_dn, 1));
                    space.TUI_clue = "- scale Y";
                    break;
                //------------------------------------------------------------
                case '-':
                    space.current_frame().scale_selection(LaserBoy_3D_double(1, 1, space.magnitude_step_up));
                    space.TUI_clue = "+ scale Z";
                    break;
                //------------------------------------------------------------
                case '_':
                    space.current_frame().scale_selection(LaserBoy_3D_double(1, 1, space.magnitude_step_dn));
                    space.TUI_clue = "- scale Z";
                    break;
                //------------------------------------------------------------
                case '=':
                    space.current_frame().scale_selection(LaserBoy_3D_double(space.magnitude_step_up, space.magnitude_step_up, space.magnitude_step_up));
                    space.TUI_clue = "+ scale XYZ";
                    break;
                //------------------------------------------------------------
                case '+':
                    space.current_frame().scale_selection(LaserBoy_3D_double(space.magnitude_step_dn, space.magnitude_step_dn, space.magnitude_step_dn));
                    space.TUI_clue = "- scale XYZ";
                    break;
                //------------------------------------------------------------
                case 'v':
                    space.current_frame().scale_selection_on_fulcrum(LaserBoy_3D_double(space.magnitude_step_up, 1, 1));
                    space.TUI_clue = "+ scale X by fulcrum";
                    break;
                //------------------------------------------------------------
                case 'V':
                    space.current_frame().scale_selection_on_fulcrum(LaserBoy_3D_double(space.magnitude_step_dn, 1, 1));
                    space.TUI_clue = "- scale X by fulcrum";
                    break;
                //------------------------------------------------------------
                case 'b':
                    space.current_frame().scale_selection_on_fulcrum(LaserBoy_3D_double(1, space.magnitude_step_up, 1));
                    space.TUI_clue = "+ scale Y by fulcrum";
                    break;
                //------------------------------------------------------------
                case 'B':
                    space.current_frame().scale_selection_on_fulcrum(LaserBoy_3D_double(1, space.magnitude_step_dn, 1));
                    space.TUI_clue = "- scale Y by fulcrum";
                    break;
                //------------------------------------------------------------
                case 'n':
                    space.current_frame().scale_selection_on_fulcrum(LaserBoy_3D_double(1, 1, space.magnitude_step_up));
                    space.TUI_clue = "+ size Z by fulcrum";
                    break;
                //------------------------------------------------------------
                case 'N':
                    space.current_frame().scale_selection_on_fulcrum(LaserBoy_3D_double(1, 1, space.magnitude_step_dn));
                    space.TUI_clue = "- scale Z by fulcrum";
                    break;
                //------------------------------------------------------------
                case 'm':
                    space.current_frame().scale_selection_on_fulcrum(LaserBoy_3D_double(space.magnitude_step_up, space.magnitude_step_up, space.magnitude_step_up));
                    space.TUI_clue = "+ scale XYZ by fulcrum";
                    break;
                //------------------------------------------------------------
                case 'M':
                    space.current_frame().scale_selection_on_fulcrum(LaserBoy_3D_double(space.magnitude_step_dn, space.magnitude_step_dn, space.magnitude_step_dn));
                    space.TUI_clue = "- scale XYZ by fulcrum";
                    break;
                //------------------------------------------------------------
                case 'a':
...
void LaserBoy_SDL_GUI::draw_polygon_menu()
{
    ...
    int number_of_sides = display_prompt_and_echo_int("number of sides [8]", 8);
    if(prompt_escaped)
        return;
...
void LaserBoy_SDL_GUI::draw_polystar_menu()
{
    ...
    space.clean_screen();
    space.render_space();
    int number_of_points = display_prompt_and_echo_int("number of points [9]", 9);
    if(prompt_escaped)
        return;
    ...
    while(bad_key)
    {
        ...
        switch(event.key.keysym.unicode)
        {
            ...
            case '1': // replace
                ...
                display_space();
                if(!space.from_ild_file(LASERBOY_ILD_SHARE + file_name)) // open failed
                {
            ...
            case '2': // prepend
                ...
                display_space();
                { // scope
                    LaserBoy_frame_set prepend(&space);
                    if(!prepend.from_ild_file(LASERBOY_ILD_SHARE + file_name))
                    {
                        display_error(LASERBOY_ILD_SHARE + file_name + " failed to open");
                        wait_4_key();
                        return false;
            ...
            case '3': // insert
                ...
                { // scope
                    ...
                    LaserBoy_frame_set insert(&space),
                                       result(&space);
                    if(!insert.from_ild_file(LASERBOY_ILD_SHARE + file_name))
                    {
                        display_error(LASERBOY_ILD_SHARE + file_name + " failed to open");
                        wait_4_key();
                        return false;
            ...
            case '4': // append
                ...
                display_space();
                if(!space.from_ild_file(LASERBOY_ILD_SHARE + file_name, true))
                {
                    display_error(LASERBOY_ILD_SHARE + file_name + " failed to open");
                    wait_4_key();
                    return false;
            ...
            case '5': // superimpose
                ...
                display_space();
                if(!space.superimpose_from_ild_file(LASERBOY_ILD_SHARE + file_name))
                {
                    display_error(LASERBOY_ILD_SHARE + file_name + " failed to open");
                    wait_4_key();
                    return false;
    ...
    while(bad_key)
    {
        ...
        switch(event.key.keysym.unicode)
        {
            ...
            case '1': // open dxf
                ...
                space.clean_screen();
                display_space();
                if(!space.from_dxf_file(LASERBOY_DXF_SHARE + file_name))
                {
            ...
            case '2': // append dxf
                ...
                space.clean_screen();
                display_space();
                if(!space.from_dxf_file(LASERBOY_DXF_SHARE + file_name, true))
                {
                    display_error(LASERBOY_DXF_SHARE + file_name + " failed to open");
                    wait_4_key();
                    break;
    ...
    while(bad_key)
    {
        ...
        switch(event.key.keysym.unicode)
        {
            ...
            case '1': // replace frame set with LaserBoy formatted wave
                ...
                {
                    if(space.error == LASERBOY_INCOMPATIBLE_FORMATS)
    ...
    while(bad_key)
    {
        ...
        switch(event.key.keysym.unicode)
        {
            ...
            case '1': // replace
                ...
                display_space();
                if(!space.from_txt_file(LASERBOY_TXT_SHARE + file_name)) // open failed
                {
            ...
            case '2': // prepend
                ...
                { // scope
                    LaserBoy_frame_set prepend(&space);
                    if(!prepend.from_txt_file(LASERBOY_TXT_SHARE + file_name))
                    {
                        display_error(LASERBOY_TXT_SHARE + file_name + " failed to open");
                        wait_4_key();
                        return false;
            ...
            case '3': // insert
                ...
                { // scope
                    ...
                    if(!insert.from_txt_file(LASERBOY_TXT_SHARE + file_name))
                    {
                        display_error(LASERBOY_TXT_SHARE + file_name + " failed to open");
                        wait_4_key();
                        return false;
            ...
            case '4': // append
                ...
                display_space();
                if(!space.from_txt_file(LASERBOY_TXT_SHARE + file_name, true))
                {
                    display_error(LASERBOY_TXT_SHARE + file_name + " failed to open");
                    wait_4_key();
                    return false;
            ...
            case '5': // superimpose
                ...
                display_space();
                if(!space.superimpose_from_txt_file(LASERBOY_TXT_SHARE + file_name))
                {
                    display_error(LASERBOY_TXT_SHARE + file_name + " failed to open");
                    wait_4_key();
                    return false;
            ...
            case '6': // superimpose
                ...
                display_space();
                if(!space.palettes_from_txt_file(LASERBOY_TXT_SHARE + file_name))
                {
                    display_error(LASERBOY_TXT_SHARE + file_name + " failed to open");
                    wait_4_key();
                    return false;
        ...
        switch(event.key.keysym.unicode)
        {
            ...
            case '6':
                  ...
                  )
                {
                    ...
                    space.render_space();
                    space.overlay_file_list();
                    display_error(LASERBOY_WTF_SHARE + file_name + " not overwritten! data not saved!");
                    wait_4_key();
                    break;
                }
                else
        ...
        switch(event.key.keysym.unicode)
        {
            ...
            case '1':
                  ...
                  )
                {
                    ...
                    space.render_space();
                    space.overlay_file_list();
                    display_error(LASERBOY_ILD_SHARE + file_name + " not overwritten! data not saved!");
                    wait_4_key();
                    break;
                }
                else
            ...
            case '4':
                ...
                {
                    temp_int = space.call_frame_effect(file_name);
                    if(temp_int != LASERBOY_OK)
                    {
            ...
            case '5':
                ...
                {
                    temp_int = space.call_frame_set_effect(file_name);
                    if(temp_int != LASERBOY_OK)
                    {
                        switch(temp_int)
                        {
                            default:
                                display_error(file_name + " is not a registered LaserBoy frame set effect");
                                break;
                            case LASERBOY_NO_SELECTED_FRAMES:
        ...
        switch(event.key.keysym.unicode)
        {
            ...
            case '1':
                  ...
                  )
                {
                    ...
                    space.render_space();
                    space.overlay_file_list();
                    display_error(LASERBOY_DXF_SHARE + file_name + " not overwritten! data not saved!");
                    wait_4_key();
                    break;
                }
                else
            ...
            case '2':
                ...
                if(directory_exists(LASERBOY_DXF_SHARE + file_name))
                {
                    ...
                    space.render_space();
                    space.overlay_file_list();
                    display_error(LASERBOY_DXF_SHARE + file_name + " exists! can not be overwritten! data not saved!");
                    wait_4_key();
                    break;
                }
                else
        ...
        switch(event.key.keysym.unicode)
        {
            ...
            case '1':
                  ...
                  )
                {
                    ...
                    space.render_space();
                    space.overlay_file_list();
                    display_error(LASERBOY_WAV_SHARE + file_name + " not overwritten! data not saved!");
                    wait_4_key();
                    break;
                }
                else
            ...
            case '5':
                ...
                else
                {
                    ...
                    if(space.auto_apply_offsets && space.have_offsets())
        ...
        switch(event.key.keysym.unicode)
        {
            ...
            case '1':
                  ...
                  )
                {
                    ...
                    space.render_space();
                    space.overlay_file_list();
                    display_error(LASERBOY_TXT_SHARE + file_name + " not overwritten! data not saved!");
                    wait_4_key();
                    break;
                }
                else
        ...
        switch(event.key.keysym.unicode)
        {
            ...
            case '1':
                  ...
                  )
                {
                    ...
                    space.render_space();
                    space.overlay_file_list();
                    display_error(LASERBOY_BMP_SHARE + file_name + " not overwritten! data not saved!");
                    wait_4_key();
                    break;
                }
                else
            ...
            case '2':
                ...
                if(directory_exists(LASERBOY_BMP_SHARE + file_name))
                {
                    ...
                    space.render_space();
                    space.overlay_file_list();
                    display_error(LASERBOY_BMP_SHARE + file_name + " exists! can not be overwritten! data not saved!");
                    wait_4_key();
                    break;
                }
                else
...
//############################################################################
void LaserBoy_SDL_GUI::display_rescale_file_error(int error)
{
    switch(error)
    {
...
int LaserBoy_SDL_GUI::start_menu_loop()
{
    ...
    if(space.from_ild_file(LASERBOY_ILD_SHARE + "in.ild"))
